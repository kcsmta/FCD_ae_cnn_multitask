...
def run_net(probs, nb_epoch = params.nb_epoch):
    ...
    branch_1 = Conv1D(filters=embddingsize, kernel_size=filter_length, strides=1, padding='same',
               dilation_rate=1, use_bias=True, activation=activation, name='decode_layer')(x)

    branch_2 = GlobalMaxPooling1D()(encode)
    ...
    model = Model(inputs=input, outputs=[branch_1, branch_2])
    ...
    print('Training...')
    best_weights = ModelCheckpoint('best_ae_cnn_multi_task_NoPretrain.h5', verbose=1, monitor='val_predict_layer_accuracy',
    ...
    plot_training_process.plot_training_process(hist.history, results_path, prob, data_part, net="ae_cnn_multi_task_NoPretrain")
    # save model
    # model.save(os.path.join(model_path, "ae_cnn_" + prob + "_" + str(int(data_part * 100)) + ".h5"))
    import shutil
    shutil.copyfile('best_ae_cnn_multi_task_NoPretrain.h5',
                    os.path.join(model_path, "ae_cnn_multi_task_NoPretrain_" + prob + "_" + str(int(data_part * 100)) + ".h5"))
    ...
    model = load_model(os.path.join(model_path, "ae_cnn_multi_task_NoPretrain_" + prob + "_" + str(int(data_part * 100)) + ".h5"))
    ...
    fout = open(os.path.join(results_path, 'ae_cnn_multi_task_NoPretrain_' + prob + "_" + str(int(data_part * 100)) + '.roc'), 'w')
    fout.write('label, ')
    ...
    print("ROC's saved to {}".format(
        os.path.join(results_path, 'ae_cnn_multi_task_NoPretrain_' + prob + "_" + str(int(data_part * 100)) + '.roc')))
    fout.close()
