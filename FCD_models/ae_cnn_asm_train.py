...
def run_net(probs, nb_epoch = params.nb_epoch):
    ...
    base_model = load_model(os.path.join(model_path, "ae_encoder_" + prob + ".h5"))
    ...
    print('Training...')
    best_weights = ModelCheckpoint('best_ae_cnn.h5', verbose=1, monitor='val_accuracy', save_best_only=True,
                                   mode='auto')
    ...
    plot_training_process.plot_training_process(hist.history, results_path, prob, data_part, net="ae_cnn")
    # save model
    # model.save(os.path.join(model_path, "ae_cnn_" + prob + "_" + str(int(data_part * 100)) + ".h5"))
    import shutil
    shutil.copyfile('best_ae_cnn.h5',
                    os.path.join(model_path, "ae_cnn_" + prob + "_" + str(int(data_part * 100)) + ".h5"))
    ...
    model = load_model(os.path.join(model_path, "ae_cnn_" + prob + "_" + str(int(data_part * 100)) + ".h5"))
    ...
    fout = open(os.path.join(results_path, 'ae_cnn_' + prob + "_" + str(int(data_part * 100)) + '.roc'), 'w')
    fout.write('label, ')
    ...
    print("ROC's saved to {}".format(
        os.path.join(results_path, 'ae_cnn_' + prob + "_" + str(int(data_part * 100)) + '.roc')))
    fout.close()
